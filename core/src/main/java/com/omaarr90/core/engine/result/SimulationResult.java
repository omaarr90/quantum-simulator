 ...
 *
 * <p>This interface provides a common contract for all simulation results while allowing different
 * engine implementations to extend with backend-specific details. The sealed nature ensures type
 * safety and enables pattern matching.
 *
 * <p>All implementations must provide measurement counts and shot information, which are the
 * fundamental outputs of any quantum simulation.
 *
...
public sealed interface SimulationResult
     ...
     *
     * <p>This constant is used by no-op engines or when no simulation is performed. It represents a
     * valid but empty result state.
     */
     ...
     *
     * <p>For deterministic simulations (single shot), each outcome will have a count of 1. For
     * probabilistic simulations with multiple shots, the counts represent the frequency of each
     * outcome across all shots.
     *
     ...
     *
     * <p>For single-shot simulations, this returns 1. For multi-shot simulations, this returns the
     * total number of circuit executions performed.
     *
