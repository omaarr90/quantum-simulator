 ...
 *
 * <p>Tests verify correct chunking logic, slice creation, parallel execution coordination,
 * and fallback behavior for small circuits.
 */
class ParallelSweepTest {
    ...
    void testSlice_EvenDivision() {
        ...
        // Note: SIMD alignment may adjust slice boundaries, so we test properties rather than exact values
    ...
    void testSlice_InvalidInputs() {
        assertThrows(IllegalArgumentException.class, 
                () -> ParallelSweep.slice(1000, 0));
        
        assertThrows(IllegalArgumentException.class, 
                () -> ParallelSweep.slice(1000, -1));
    ...
    void testForEachSlice_SerialExecution() throws InterruptedException {
        ...
        ParallelSweep.forEachSlice(stateVector, 10, slice -> {
    ...
    void testForEachSlice_ParallelExecution() throws InterruptedException {
        ...
        ParallelSweep.forEachSlice(stateVector, 15, slice -> {
            callCount.incrementAndGet();
            totalElements.addAndGet(slice.length());
    ...
    void testForEachSlice_InterruptionHandling() {
        ...
        RuntimeException thrown = assertThrows(RuntimeException.class, () -> {
            ParallelSweep.forEachSlice(stateVector, 15, slice -> {
                Thread.currentThread().interrupt();
                try {
                    Thread.sleep(100); // This should throw InterruptedException
                } catch (InterruptedException e) {
        ...
        for (int size : testSizes) {
            for (int sliceCount : testSliceCounts) {
                if (size >= sliceCount) { // Valid combination
                    ...
                    assertEquals(size, slices.get(slices.size() - 1).end(), 
