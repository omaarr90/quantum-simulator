...
public class AstToCircuitVisitor extends OpenQasm3BaseVisitor<Circuit> {
    ...
    public Circuit visitClassicalDeclaration(OpenQasm3Parser.ClassicalDeclarationContext ctx) {
        ...
        if (classicalMap.containsKey(cregName)) {
            throw createParseException("Classical register '" + cregName + "' already declared", ctx);
        }
    ...
    public Circuit visitGateApplication(OpenQasm3Parser.GateApplicationContext ctx) {
        ...
        switch (gateName.toLowerCase()) {
            case "h":
                if (qubits.length != 1) throw createParseException("H gate requires exactly 1 qubit", ctx);
                builder.h(qubits[0]);
                break;
            case "x":
                if (qubits.length != 1) throw createParseException("X gate requires exactly 1 qubit", ctx);
                builder.x(qubits[0]);
                break;
            case "y":
                if (qubits.length != 1) throw createParseException("Y gate requires exactly 1 qubit", ctx);
                builder.y(qubits[0]);
                break;
            case "z":
                if (qubits.length != 1) throw createParseException("Z gate requires exactly 1 qubit", ctx);
                builder.z(qubits[0]);
                break;
            case "cx":
            case "cnot":  // Alias for cx
                if (qubits.length != 2) throw createParseException("CX/CNOT gate requires exactly 2 qubits", ctx);
                builder.cx(qubits[0], qubits[1]);
                break;
            case "cz":
                if (qubits.length != 2) throw createParseException("CZ gate requires exactly 2 qubits", ctx);
                builder.cz(qubits[0], qubits[1]);
                break;
            case "swap":
                if (qubits.length != 2) throw createParseException("SWAP gate requires exactly 2 qubits", ctx);
                builder.swap(qubits[0], qubits[1]);
                break;
            case "rx":
                if (qubits.length != 1) throw createParseException("RX gate requires exactly 1 qubit", ctx);
                double rxAngle = evaluateExpression(ctx.gateCall().expression());
                builder.rx(qubits[0], rxAngle);
                break;
            case "ry":
                if (qubits.length != 1) throw createParseException("RY gate requires exactly 1 qubit", ctx);
                double ryAngle = evaluateExpression(ctx.gateCall().expression());
                builder.ry(qubits[0], ryAngle);
                break;
            case "rz":
                if (qubits.length != 1) throw createParseException("RZ gate requires exactly 1 qubit", ctx);
    ...
    private int getClassicalIndex(OpenQasm3Parser.ClassicalReferenceContext ctx) {
        ...
        if (!classicalMap.containsKey(cbitName)) {
            // If not declared, assume it's an implicit classical register (for backward compatibility)
    ...
    private double evaluateExpression(OpenQasm3Parser.ExpressionContext ctx) {
        if (ctx == null) {
            throw new RuntimeException(new ParseException("Missing expression for parameterized gate"));
        }
        ...
        if (ctx.REAL() != null) {
            ...
            switch (op) {
                ...
                case "*": return left * right;
                case "/": return left / right;
                default: throw createParseException("Unsupported operator: " + op, ctx);
            }
    ...
    private RuntimeException createParseException(String message, org.antlr.v4.runtime.ParserRuleContext ctx) {
        ...
        return new RuntimeException(new ParseException(message, line, column, offendingSymbol, suggestion));
    }
