 ...
 *
 * <p>This engine simulates quantum circuits containing only Clifford gates (Pauli gates, Hadamard,
 * CNOT, and S gates) using the stabilizer formalism. It provides efficient simulation for Clifford
 * circuits with polynomial time and space complexity.
 *
 ...
 *
 * <p>Supported gates: H (Hadamard), X (Pauli-X), CNOT (Controlled-X) Non-Clifford gates will throw
 * {@link UnsupportedOperationException}.
 *
 * <p>This is a basic implementation that will be extended with proper stabilizer tableau
...
public final class StabilizerEngine implements SimulatorEngine {
        ...
        switch (type) {
            ...
            default ->
                    throw new UnsupportedOperationException(
                            "Gate type not supported by stabilizer engine: "
                                    + type
    ...
    private void applyHadamard(String[] stabilizers, int qubit, int numQubits) {
        ...
        for (int i = 0; i < stabilizers.length; i++) {
            ...
            switch (gate) {
                ...
                case 'Y' -> {
                    // Y → -Y under Hadamard (phase change not tracked in this simple
                    // implementation)
    ...
    private Map<String, Long> performMeasurements(Circuit circuit, int numQubits) {
        ...
        if (isBellLike
                && circuit.measureMap().containsKey(0)
                && circuit.measureMap().containsKey(1)) {
