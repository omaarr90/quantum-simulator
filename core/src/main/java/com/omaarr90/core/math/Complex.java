 ...
 *
 * <p>Represents a complex number z = a + bi where a and b are real numbers, and i is the imaginary
 * unit (i² = -1).
 *
 * <p><strong>IMMUTABILITY GUARANTEE:</strong> This class is completely immutable. Every arithmetic
 * operation ({@link #add}, {@link #sub}, {@link #mul}, {@link #div}, {@link #scale}, {@link
 * #conjugate}) returns a <strong>new</strong> {@link Complex} instance, leaving the original
 * unchanged. No method modifies the state of existing instances.
 *
 * <p>Designed for intrinsic friendliness: only primitive {@code double} fields, no boxing, and a
 * tiny record that the JIT can scalar‑replace. All operations return new {@link Complex} instances
 * so callers can fluently chain arithmetic without hidden allocations.
 */
public record Complex(double real, double imaginary) {
     ...
     *
     * <p>Complex division: (a+bi)/(c+di) = ((ac+bd) + (bc-ad)i)/(c²+d²)
     *
    ...
    /**
     * Squared magnitude (|z|²) – avoids the cost of a square root. Ideal for tight loops and SIMD
     * reductions.
     *
    ...
    /**
     * Euclidean magnitude |z| (also called modulus or absolute value). Uses {@link
     * Math#hypot(double, double)} for numerical stability.
     *
     * @return |z| = √(real² + imaginary²)
     */
    public double abs() {
    ...
    /**
     * Argument (phase angle) of the complex number. Returns the angle θ in polar representation z =
     * r*e^(iθ).
     *
