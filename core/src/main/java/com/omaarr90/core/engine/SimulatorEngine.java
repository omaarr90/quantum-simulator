 ...
 *
 * <p>This interface defines the contract for quantum circuit simulation backends. Implementations
 * can provide different simulation strategies such as state-vector, stabilizer, tensor-network, or
 * other quantum simulation approaches.
 *
 * <p>Implementations should be:
 *
 * <ul>
 *   <li>stateless & thread-safe <strong>or</strong> clearly document thread-safety
 *   <li>deterministic for a fixed PRNG seed (if noise is involved)
 *   <li>able to throw {@link UnsupportedOperationException} for unsupported gates
 * </ul>
 *
 * <p>Engines are discovered at runtime via Java's {@link java.util.ServiceLoader}. Each
 * implementation must be registered in {@code
 * META-INF/services/com.omaarr90.qsim.engine.SimulatorEngine}.
 *
...
public interface SimulatorEngine {
     ...
     *
     * <p>The implementation should process all gate operations in the circuit and perform
     * measurements as specified in the circuit's measurement map.
     *
     ...
     *
     * <p>This identifier is used by {@link SimulatorEngineRegistry} to look up specific engine
     * implementations.
     *
