...
/**
 * Immutable representation of a quantum circuit. Contains all the information needed to describe a
 * complete quantum circuit including gates, measurements, and metadata.
 *
 * <p>This record is thread-safe and can be safely shared between threads. All collections are
 * immutable to guarantee thread safety.
 *
    ...
    public Circuit {
        ...
        if (classicalBits < 0) {
            throw new IllegalArgumentException(
                    "Classical bits count cannot be negative: " + classicalBits);
        }
        if (ops == null) {
        ...
        for (Map.Entry<Integer, Integer> entry : measureMap.entrySet()) {
            ...
            if (qubit < 0 || qubit >= qubitCount) {
                throw new IllegalArgumentException(
                        "Invalid qubit index in measurement map: "
                                + qubit
                                + " (circuit has "
                                + qubitCount
                                + " qubits)");
            ...
            if (cbit < 0 || cbit >= classicalBits) {
                throw new IllegalArgumentException(
                        "Invalid classical bit index in measurement map: "
                                + cbit
                                + " (circuit has "
                                + classicalBits
                                + " classical bits)");
        ...
        for (GateOp op : ops) {
            if (op instanceof GateOp.Gate gate) {
                for (int qubit : gate.qubits()) {
                    if (qubit < 0 || qubit >= qubitCount) {
                        throw new IllegalArgumentException(
                                "Invalid qubit index in gate operation: "
                                        + qubit
                                        + " (circuit has "
                                        + qubitCount
                                        + " qubits)");
                ...
                if (barrier.qubits().length > 0) {
                    for (int qubit : barrier.qubits()) {
                        if (qubit < 0 || qubit >= qubitCount) {
                            throw new IllegalArgumentException(
                                    "Invalid qubit index in barrier operation: "
                                            + qubit
                                            + " (circuit has "
                                            + qubitCount
                                            + " qubits)");
