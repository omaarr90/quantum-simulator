 ...
 *
 * <p>This class provides the core infrastructure for parallelizing quantum gate operations
 * using Java 21's StructuredTaskScope. It handles chunking of the amplitude space into
 * cache-aligned slices and coordinates parallel execution across multiple threads.
 *
 ...
 * </ul>
 *
 * <p><strong>THREAD SAFETY:</strong> All methods are thread-safe. The parallel execution
 * operates on disjoint memory regions, requiring no synchronization between worker threads.
 */
public final class ParallelSweep {
    ...
    public static List<AmplitudeSlice> slice(int nAmps, int slices) {
        ...
        if (nAmps < slices) {
            throw new IllegalArgumentException(
                    "Cannot create more slices than amplitudes: nAmps=" + nAmps + ", slices=" + slices);
        }
     ...
     * StructuredTaskScope.ShutdownOnFailure. Each slice is processed by a separate
     * thread, with proper error handling and cancellation semantics.
     *
     * <p><strong>ERROR HANDLING:</strong> If any slice fails, all other slices are
    ...
    public static String describeSlicing(int nAmps, int nQubits) {
        ...
        sb.append(String.format("Slicing %d amplitudes (%d qubits) into %d slices:\n", 
