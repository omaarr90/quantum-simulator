 ...
 * 
 * <p>This utility class provides a convenient interface for working with simulation engines
 * that are registered through Java's Service Provider Interface (SPI). Engines are
 * automatically discovered at runtime from the classpath.
 * 
 * <p>Engine implementations must be registered in
 * {@code META-INF/services/com.omaarr90.qsim.engine.SimulatorEngine} files.
 * 
...
public final class SimulatorEngineRegistry {
     ...
     * 
     * <p>This method scans the classpath for registered engine implementations
     * and returns their unique identifiers. The returned set is immutable.
     * 
     ...
     * 
     * <p>This method searches for an engine implementation with the given ID.
     * Engine IDs are case-sensitive and should match exactly.
     * 
        ...
        synchronized (LOADER) {
            return LOADER.stream()
                    ...
                    .orElseThrow(() -> new NoSuchElementException(
                            "No simulation engine found with ID: " + id + 
     ...
     * 
     * <p>This method is useful when you need any available engine and don't care
     * about the specific implementation. The order is not guaranteed and may
    ...
    public static SimulatorEngine first() {
        synchronized (LOADER) {
            return LOADER.stream()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new NoSuchElementException(
                            "No simulation engines are available. " +
                            "Make sure engine implementations are on the classpath " +
                            "and properly registered in META-INF/services."));
        }
    }
     ...
     * 
     * <p>This method is useful in dynamic environments where engine implementations
     * might be added to the classpath at runtime. In most cases, this method
     * is not needed as the ServiceLoader will automatically discover engines.
     */
