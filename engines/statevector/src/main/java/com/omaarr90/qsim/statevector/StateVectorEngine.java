 ...
 * 
 * <p>This engine simulates quantum circuits by maintaining the full quantum state vector
 * and applying unitary transformations for each gate operation. It provides exact
 * simulation results but has exponential memory requirements (2^n complex amplitudes
 * for n qubits).
 * 
 ...
 * Measurements are performed by sampling from the probability distribution
 * defined by the quantum state amplitudes.
 */
public final class StateVectorEngine implements SimulatorEngine {
    ...
    public SimulationResult run(Circuit circuit) {
        ...
        if (numQubits > 20) { // Reasonable limit to prevent memory issues
            throw new IllegalArgumentException(
                "Circuit has too many qubits for state-vector simulation: " + numQubits + 
    ...
    private Map<String, Long> performMeasurements(double[] amplitudes, Circuit circuit, int numQubits) {
