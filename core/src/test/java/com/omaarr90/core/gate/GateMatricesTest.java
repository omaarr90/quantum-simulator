...
class GateMatricesTest {
    ...
    private void assertMatricesEqual(Complex[][] expected, Complex[][] actual, String message) {
        ...
        for (int i = 0; i < expected.length; i++) {
            for (int j = 0; j < expected[0].length; j++) {
                ...
                assertEquals(
                        expected[i][j].imaginary(),
                        actual[i][j].imaginary(),
                        TOLERANCE,
            ...
            for (double angle : testAngles) {
                ...
                assertEquals(
                        ...
                        gateType.name()
                                + "("
                                + angle
                                + ") matrix determinant magnitude should be 1");
            }
        }
    }
    ...
    void testToGateMethods() {
        ...
        assertEquals(
                GateType.SDG,
                ((FixedGate) sdgGate).type(),
                "SDG FixedGate should have correct type");

        Gate tdgGate = GateType.TDG.toGate();
        assertTrue(tdgGate instanceof FixedGate, "TDG.toGate() should return FixedGate");
        assertEquals(
                GateType.TDG,
                ((FixedGate) tdgGate).type(),
                "TDG FixedGate should have correct type");
        ...
        assertTrue(
                rxGate instanceof ParameterizedGate,
                "RX.toGate(theta) should return ParameterizedGate");
        assertEquals(
                GateType.RX,
                ((ParameterizedGate) rxGate).type(),
                "ParameterizedGate should have correct type");
        assertEquals(
                theta,
                ((ParameterizedGate) rxGate).theta(),
                1e-10,
                "ParameterizedGate should have correct theta");
        ...
        assertEquals(
                GateType.H,
                ((FixedGate) hGateWithTheta).type(),
                "FixedGate should have correct type");

        // Test matrix consistency - gates created via toGate() should have same matrices as direct
        // construction
    ...
    private Complex calculateDeterminant(Complex[][] matrix) {
        ...
        if (size == 2) {
            ...
            throw new IllegalArgumentException(
                    "Determinant calculation only supports 2x2 and 4x4 matrices");
        }
    }
    ...
    private Complex calculateCofactor(Complex[][] matrix, int row, int col) {
        ...
        return minor[0][0]
                .mul(minor[1][1].mul(minor[2][2]).sub(minor[1][2].mul(minor[2][1])))
                .sub(
                        minor[0][1].mul(
                                minor[1][0].mul(minor[2][2]).sub(minor[1][2].mul(minor[2][0]))))
                .add(
                        minor[0][2].mul(
                                minor[1][0].mul(minor[2][1]).sub(minor[1][1].mul(minor[2][0]))));
    }
}
