...
class SimulatorEngineRegistryTest {
    ...
    void testAvailableEngines() {
        ...
        assertTrue(available.contains("statevector"), 
            "Should contain statevector engine. Available: " + available);
        assertTrue(available.contains("stabilizer"), 
    ...
    void testFirstEngine() {
        ...
        Set<String> available = SimulatorEngineRegistry.available();
        assertTrue(available.contains(engine.id()), 
            "First engine ID should be in available engines");
    }
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            threads[i] = new Thread(() -> {
                try {
                    ...
                    for (int j = 0; j < 100; j++) {
                        SimulatorEngine engine1 = SimulatorEngineRegistry.get("statevector");
                        SimulatorEngine engine2 = SimulatorEngineRegistry.get("stabilizer");
                        Set<String> available = SimulatorEngineRegistry.available();
    ...
    void testReload() {
        ...
        assertEquals(beforeReload, afterReload, 
            "Available engines should be the same after reload");
    }
}